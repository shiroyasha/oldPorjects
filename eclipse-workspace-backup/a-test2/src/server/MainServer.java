...
public class MainServer  extends UnicastRemoteObject implements MessageServerI,AgentServerI{
	...
	*/
	...
	/*
	 * treci konstruktor, prima host, ukoliko se ne radi na localhostu
	 */
	public MainServer(String serverName,GlavniGui gui,String host,ServerInfo[] servers,String[] racers) throws RemoteException{
		serverInfo=new ServerInfo(serverName,"1099",host);
		registry = LocateRegistry.createRegistry(1099);
		registry.rebind("AgentServer", this);
		System.out.println("Main Server created on "+host+":1099");
		this.gui=gui;
		finished=0;
		started=racers.length;
		times=new HashMap<String,Integer>();
	...
	//zbog ponovnog pokretanja igre u toku rada programa
	public void reset(ServerInfo[] servers,String[] racers) throws RemoteException
	{
		finished=0;
		started=racers.length;
		times=new HashMap<String,Integer>();
	...
	*/
	...
	{
		System.out.println("agent "+rpi.getIndexOfAgent()+" se nalazi na serveru "+rpi.getServerInfo().host+":"+rpi.getServerInfo().port+" zavrsio ? "+rpi.finished()+" vrijeme "+System.currentTimeMillis());
