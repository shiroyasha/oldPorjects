...
public class GlavniGui extends JFrame
{
	...
	public GlavniGui( List<ServerInfo> serveri , String localhost ) 
	{
	...
	public void startRace() 
	{
		...
		{
			if( mainServer != null ) 
				mainServer.reset(serverSetupPage.getServerList().toArray( new ServerInfo[0] ),  agentSetupPage.getAgentList());
			else 
				mainServer = new MainServer("", this, localhost, serverSetupPage.getServerList().toArray( new ServerInfo[0] ), agentSetupPage.getAgentList() );
			
			mainServer.startRace();
		}
		catch(Exception e)
		{
			e.printStackTrace();
			System.out.println("FATAL ERROR ^_^");
			System.exit(0);
		}
	}
     ...
     */  
	public void raceFinished( String[] imena, Integer[] vreme )
	{
		raceResultsPage.setResults( imena, vreme );
		raceViewPage.raceFinished();
	}
