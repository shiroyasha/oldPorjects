...
public class ServerOperations implements SOI {
		...
		try {
			...
			if (found != null)
				if (found.getPassword().equals(pass)) {
					...
					found.setTimeSinceLastActivity(TIMEOUT);
					em.merge(found);
					return sessionId;
				}
		} catch (Exception e) {
	...
	public int register(String ime, String username, String password, int idTipaKorisnika) {
		try {
			...
			Korisnik e = new Korisnik(ime, username, password, foundTip);
	...
	public double addToCart(String sessionId, String itemName) {
		try {
			...
			if (art == null || kor == null)
				return 0;
			kor.setTimeSinceLastActivity(TIMEOUT);
			em.merge(kor);
		...
		try {
			...
			if (listKupljeno.size() > 0) {
				...
				for (Kupljeno k : listKupljeno) {
					data[i++] = k.getArtikal().getId() + ";" + k.getKorisnik().getUsername() +
								";" + k.getKomentar();
				}
				return data;
			}
	...
	public int[] getRatingsFor(String itemName) {
		try {
			...
			if (listKupljeno.size() > 0) {
				int i = 0;
				int sum = 0;
				int[] data = new int[listKupljeno.size() + 1];
				for (Kupljeno k : listKupljeno) {
					data[i++] = k.getOcena();
					sum += k.getOcena();
				}
				data[i] = sum / listKupljeno.size();
				return data;
			}
		} catch (Exception e) { return null; }
		return null;
