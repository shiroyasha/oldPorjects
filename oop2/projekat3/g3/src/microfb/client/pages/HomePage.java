...
public class HomePage extends AbstractPage {
	...
	private void loadLeft(final String username) {
		...
		loading.setVisible(true);
		left.add(loading);
		left.add(new HTML("<br/>"));
		
		final ButtonFB home = new ButtonFB("My Profile");
	...
	private void loadRight(final KorisnikIO kor) {
		...
		tester.scheduleRepeating(REFRESH_TIME);
		right.add(profile);
		...
		final Timer tester = new Timer() {
			...
			public void run() {
				if (cover.getWidth() < 20 || cover.getHeight() < 20)
					cover.setUrl("/uploadService?sessionID=" + FBEntryPoint.sessionID +"&email=" + kor.getEmail() + "&type=cover");
				else
					if (!resized.get()) {
