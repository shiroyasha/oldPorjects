...
public class UploadServlet extends HttpServlet
{
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException
	{
		...
		try
		{
			...
			if( slika != null)
			{
				System.out.println( "velicina slike " + slika.length );
			...
			else if( type.equals("cover") )
			{
				...
				resp.sendRedirect("/images/no-cover.jpg");
			}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if( sessionID == null || email == null || type == null ) throw new IOException("wrong arguments");
		System.out.println("updating " + type + " photo");
		if( !Service.loggedIn(sessionID) ) throw new IOException("not logged in");
		...
		try{
			...
			while (iter.hasNext()) {
				...
				System.out.println("updajting " + type + " photo " + out.size() );
				if(out.size() == 0 ) 
				{
					throw new RuntimeException("File size is == " + out.size() );
				}
